function [mi, state]=rsFindVesiclesX(m,mi,rPars,findInMask)
% Derived from rsFindVesicles3
% Calls:
...
%     Make a Gaussian HP filter
    ...
    if ~(isfield(t,'rPars') && all(t.rPars==rPars) ...
        ...
        t.sfrefs=single(zeros([ns,t.nrsteps]));
        t.spowers=zeros(t.nrsteps,1);
        t.srrefs=single(zeros([ns t.nrsteps]));
        t.sorefs=single(zeros([ns t.nrsteps]));
        t.sopowers=zeros(t.nrsteps,1);
        ...
        for i=1:t.nrsteps
            ...
            v = SphereDensity(ns,r,vesOrigin);
            fv=-H.*fftn(ifftshift(v));    % FT of vesicle at origin.
            t.sfrefs(:,:,i)=fv;
            t.sorefs(:,:,i)=v;
            t.sopowers(i)=v(:)'*v(:);
    ...
    t.sccs=single(zeros([ns t.nrsteps]));
    % cc values are scaled by 1/r^rExponent to make small vesicles as easily found
    for i=1:t.nrsteps
        t.sccs(:,:,i)=t.rads(i).^(rExponent)/t.powers(i)*...
            real(ifftn(t.fms.*conj(t.sfrefs(:,:,i))));
    end;
    % ccs=ccs-min(ccs(:));  % min value is zero.
    [t.sccmx, t.sccmi]=max(t.sccs,[],3);
    t.sccmxScaled=t.sccmx.*(t.rads(t.sccmi).^-rExponent);  % undo scaling of references
    ...
    switch lower(m)
        case 'next'
            ...
            while t.nfound<nf0+maxN
                ...
                [t.sglobalmax, sjx, sjy]=max2d(t.sccmx2);
                if t.globalmax<(thresh(1)*t.rads(1)^rExponent) && ...
                   t.sglobalmax<(thresh(1)*t.rads(1)^rExponent) % below minimum value
                    break
                end;
                isBall=t.sglobalmax>t.globalmax;
                ...
                if isBall
                    [ampi, refii]=max1di(squeeze(t.sccs(sjx,sjy,:)).*radScalings);
                else
...
%                     Also we reject radii that are at the minimum.
                    ...
                    if fracMasked<maxFracMasked && ampi>thresh(1)*fracMasked
                        ...
                        if isBall
                        t.mi.vesicle.ok(t.nfound,1:4)=[0 0 0 0];  % flag indicates a vesicle in bounds.
                        vref=ampi*circshift(t.srrefs(:,:,refi),round([sjx sjy]-ns/2-1));
                        else
