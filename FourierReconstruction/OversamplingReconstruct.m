...
% and an nim x 3 matrix of corresponding Euler angles, insert these
...
% pts=((x.^2+y.^2)<(n/2-1)^2);  % this is about the same speed.
x=x(pts);  % select only the active ones.
y=y(pts);
ct2=nx/2;  % center offset in oversampled volume

isComplex=~isreal(FourierStack);
% This can just be set false for simplicity, but for some reason the lookup
% is much slower when complex values are used (memory swapping?)
if isComplex
    FourierStack=ToCAS(FourierStack); % Convert to cosine-and-sine
end;
...
% We are rotating the coordinate system.
    ...
    % Do the accumulation here.  The fact that the oversampling is greater
    % than sqrt(3) means that, at the insertion of each plane, no point in
    % the volume is affected by more than one plane pixel.  This makes the
    % accumulation code in Matlab very simple.
    volx(P)=volx(P)+img(pts);  % This is slowest line, when numbers are complex.
%     Hence the conversion to CAS below.
end;
% toc
...
if isComplex
    volx=FromCAS(volx);  % takes 30% of total time, but worth it.
end;
rvolx=fftshift(real(ifftn(ifftshift(volx)))); % go to real space
