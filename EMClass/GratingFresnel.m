% GratingFresnel.m
% Use the Fresnel propagator to 
n=1024;
n2=n/2;
fx=100; % cycles per width
fy=100; % cycles per height
amp=.3; % extent of grating phase shift, radians
...
fy1=fy*(1-.128);
for i=1:n0-1
%     Psi(:,n0-i)=Psi(:,n0-i+1).*exp(-1i*(pi*dfz*(xs.^2)+2*pi*fy/(n+150)));
    Psi(:,n0-i)=Psi(:,n0-i+1).*exp(-1i*(pi*dfz*(xs.^2)+2*pi*fy1/n));
