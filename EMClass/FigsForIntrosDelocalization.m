% Figs for intro lectures, with big delocalization
...
ctPars.theta=0;
ctPars.Cs=2.7;
fProj=fftn(ifftshift(objC));
freqs=RadiusNorm(nC)/s.pixA;
...
ctPars.defocus=3;
    ...
    subplot(122);
    imaga(imscale(img,256,.02));
%     imags(img);
    axis off;
    
barLength=100/s.pixA;
yval=nC/15;
xvals=nC/2+barLength/2*[-1 1];
hold on;
plot(xvals,[yval yval],'w-','linewidth',2);
text(nC/2,yval+10,'100 Å','color','w','horizontalalignment','center','fontsize',24);
hold off;
return
