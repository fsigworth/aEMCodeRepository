function [out1,out2,out3] = Myginput2(arg1,pointerType,whitePointer)
...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    initialState = setupFcn(fig,pointerType,whitePointer);
...
function initialState = setupFcn(fig,pointerType,whitePointer)
...
if ~isempty(initialState.toolbar)
    ...
    elseif strcmpi(pointerType,'squareshadow')
        % Pointer map
        pn=16; %  Make a 16 x 16 square
        %         Black inner square
        range=1:pn-1;
        ...
        set(fig,'pointer','custom','pointershapecdata',ptrMap,...
            'pointershapehotspot',[1 1]*(pn/2-1));
    elseif strcmpi(pointerType,'thicksquare')
        % Pointer map
        pn=16; %  Make a 16 x 16 square
        %         Black inner square
        range=1:pn-1;
        ...
        ptrMap(1:2,range)=2;
        ptrMap(pn-2:pn-1,range)=2;
        ptrMap(range,1:2)=2;
        ptrMap(range,pn-2:pn-1)=2;
        set(fig,'pointer','custom','pointershapecdata',ptrMap,...
            'pointershapehotspot',[1 1]*(pn/2-1));
    else
