function [nTrunc,ts,fsModel,m2,rm]=WriteZTiff(m,pixA,filename,mpars)
...
if pars.lfCutoff>.2
    warning(['pars.lfCutoff seems high: ' num2str(pars.lfCutoff)]);
end;
if pars.displayOn
    figure(1);
    colormap jet
...
%     fs0=[.05 .07 .1 .13 .16 .2 .25 .4];  % Gaussian frequencies, a row vector
    fs=fs0;
    ...
    if pars.displayOn
        ...
        subplot(223); imagesc(d.*mask);  % show the log 2D spectrum-model
        colorbar;
        ...
        mskn=single(mask>.5);
        mskn(~mskn)=NaN;
        plot(freqs,[Radial(log(sp2b)) Radial(log(model)) Radial(log(model).*mskn)]/log(10));
