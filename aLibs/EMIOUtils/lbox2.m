...
q=get(handles.figure1,'SelectionType');
switch q
    case 'normal'  % just moved to another file
    ...
    filename = file_list{index_selected};
        ...
        if strcmp(ex,'.mrc')
            [m,s]=ReadMRC(filename,1,1);  % get the first slice
            if s.nz>1
                m=ReadMRC(filename,ceil((s.nz+1)/2),1); % get the central slice
            end;
            figure(2);
            imags(m);
        end;
