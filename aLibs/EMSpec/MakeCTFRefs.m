...
% function [refs, refsz]=MakeCTFRefs(n,res,P,msk)
% Compute a huge array of |CTF|^2 functions for correlation with a power
% spectrum, to implement the algorithm of CTFFIND3.  Each function is
% normalized to have unity variance (for NCC).
% The structure P has either scalars or vectors for the fields
% P.defocus, P.deltatdef, P.theta, P.alpha and P.B.
% P.deltadef is the
...
% % % %                     refs(:,:,ii)=c/sqrt(c(:)'*c(:));  % normalize variance.
                    refs(:,:,ii)=c;  % don't normalize variance.
                end;
            end;
        end;
    end;
