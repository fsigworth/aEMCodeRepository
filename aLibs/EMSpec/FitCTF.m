function [P, c, spect, disData]=FitCTF(mc,Pa,pixA,maxres,minres,test,options)
...
% Example values for maxres and minres are 6 and 20.
...
% Regardless of the test value, the disData structure contains data for
% constructing a display of the fit.
...
nuVal=3*median(Pa.defocus)/(pixA*maxres);  % nominal window size / 256
nu=min(nx/2,256*max(1,ceil(nuVal)));
% disp([num2str(nu) ' points in spectrum for ctf']);
fExponent=1;
...
if test  % Show the micrograph
    figure(1);
    SetGrayscale;
    subplot(2,3,1);
    imags((1:nx)*pixA/10,(1:ny)*pixA/10,mc);
    xlabel('nm');
    drawnow;
end;
...
for ix=1:tx
    ...
    for iy=1:ty
        ...
        % subplot(2,1,1); imags(tm); title([ix iy]);
        
        sp2=abs(fftn(tm)).^2;
        % subplot(2,1,2); imags(fftshift(sqrt(sp2)));
        % drawnow;
        sps(:,:,ix,iy)=sp2;
...
if test
    subplot(2,3,4);
    xs=(-nu/2:nu/2-1)*df;
    imags(xs,xs,fftshift(sqrsp.^(disexp*2)));
    xlabel('A^{-1}');
...
if test  % draw the graphics
    ...
    subplot(2,3,5);
    xs2=(-nu/4:nu/4-1)*df;
    imaga(xs2,xs2,Crop(dspect,nu/2));
    xlabel('A^{-1}');
...
disData=struct;  % display data
disData.img.image=GaussFilt(BinImage(mc,4),.1);
disData.img.axis='off';

disData.contour.xs=Ps.deltadef;
disData.contour.ys=Ps.defocus;
disData.contour.image=squeeze(ccx(m,l,:,:,k))';
disData.contour.xLabel='Delta-defocus';
disData.contour.yLabel='Defocus';
...
if test
    ...
    subplot(2,3,3);
    imacs(Ps.deltadef,Ps.defocus,squeeze(ccx(m,l,:,:,k))');
...
%     imaga(Crop(imscale(abs(RotSymmetrize(sqrspm,4)).^(disexp*2),256,.01),nu/2));
%     LabelImage(nu/2,df,'A^{-1}');
%     title(['CC =' num2str(c)]);
    
  % Prepare the radial display
%     rs=Radial(limits.*diffsp);
    
    rs=RadialCTF(limits.*diffsp,P,pixA);  % %%%% Use Penczek formula
    rref=RadialCTF(dref,P,pixA);
    
    rs=150*(rs-min(rs))/(max(rs)-min(rs));
%     radCorrSpecs=[rs Radial(dref)];
    plotLimits=[max(1,floor(rmin*0.9)) min(numel(freqs),ceil(rmax*1.1))];
    radCorrSpecs=[rs rref];
    radCorrSpecs=radCorrSpecs(plotLimits(1):plotLimits(2),:);
    plotFreqs=freqs(plotLimits(1):plotLimits(2));
    disData.radial.plotx=plotFreqs;
    disData.radial.ploty=radCorrSpecs;
    disData.radial.xLabel='A^{-1}';
    
    %     Prepare the spectrum display
    dspect=imscale(limits.*diffsp);
    dref=imscale(limits.*CTF(nu,pixA,P).^2);
    dspect(1:nu/2+1,:)=dref(1:nu/2+1,:);
    xs=(-nu/2:nu/2-1)*df;
    ...
    disData.splitSpectrum1.xs=xs;
    disData.splitSpectrum1.image=dspect;
    disData.splitSpectrum1.xLabel='A^{-1}';
    disData.splitSpectrum2.title=['CC =' num2str(c)];
    
    disData.splitSpectrum2=disData.splitSpectrum1;
    disData.splitSpectrum2.xs=xs2;
    disData.splitSpectrum2.image=Crop(dspect,nu/2);
    disData.splitSpectrum1.xLabel='A^{-1}';
    disData.splitSpectrum2.titleTex=['\Deltaz=' num2str(-p(1)) ...
        '   Astig=' num2str(p(2)) '   \theta=' num2str(180/pi*p(3))];
...
if test
    ...
    imaga(xs2,xs2,Crop(dspect,nu/2));
    xlabel('A^{-1}');
    title(disData.splitSpectrum2.titleTex);

    subplot(2,3,6);
    plot(plotFreqs,radCorrSpecs);
    xlabel('A^{-1}');
    drawnow;
end;
