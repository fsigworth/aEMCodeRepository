function [rm rmat]= TransformImagePar(m, theta, scale, shift, mode, blocksize)
...
nb=ceil(n/blocksize);
rm=zeros(n,n);
blksz=min(n,blocksize);
for by=0:nb-1
    ly=1+by*blocksize;
    uy=min(n,(by+1)*blocksize);
    imx=zeros(blksz,blksz,nb);
    parfor (bx=1:nb)
        lx=1+(bx-1)*blocksize;
        ux=bx*blocksize;        
        ...
        % Interpolate.
        disp([by bx]);
        imx(:,:,bx)=interp2( m, yi, xi, mode, 0);
    end;
    for bx=1:nb
        lx=1+(bx-1)*blocksize;
        ux=bx*blocksize;
        rm(lx:ux,ly:uy)=imx(:,:,bx);
    end;
end;
