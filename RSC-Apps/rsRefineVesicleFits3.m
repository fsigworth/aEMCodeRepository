...
dpars.overwrite=1;
dpars.writeMiFile=1;
dpars.doPreSubtraction=0; % subtract all vesicles except the one being fitted
...
dpars.rTerms=[90 100 110 150 200 250 inf]; %%%%
% dpars.rTerms=[150 200 inf];  % for Mengqiu
dpars.limitOrigNTerms=2; % max number of radius terms to fit first
dpars.stepNTerms=2;
dpars.nTermsForExtraRound=1; % normally 5
...
dpars.peakPositionA=[-33 0 33];  % empirical default.  Works a bit better than [37 37]
...
%                 p.limitOrigNTerms=round(pars.fractionStartingTerms(ind)*maxRTerms+1);
                ...
                p.stepNTerms=pars.stepNTerms;
                p.preserveOldRadiusTerms=pars.preserveOldRadiusTerms;
                p.nTermsForExtraRound=pars.nTermsForExtraRound;
