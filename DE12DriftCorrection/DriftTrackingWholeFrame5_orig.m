% DriftTrackingWholeFrame5
%  Track drift using the spectral model directly.
% F.S. 4 June 12, 25 Sep 12
...
showWeightingFilter=0; % option has a bug
...
% else
        [H1, rs]=deGetWeightingFilter3(wI1,wIMean,wISpec,hMask,pixA*dsw,useCrystal);
% end;
...
%         imacs(xs,ys,Crop(fftshift(H1),nw));
% xlabel('frequency, pix^{-1}');
    ...
    %%
    if showWeightingFilter % create Fig. 4 of the paper
        figure(2);
        subplot(311);
        CCNoiseFigure('/Users/fred/Documents/papers/DE Camera/V2/Figures/Data/WFigure2a.mat');
        xlabel('x-displacement','fontsize',12);
        ...
        maxf=max(f0)*1.02;
        subplot(323);
        semilogy(f0,min(1e3,max([GaussFiltDCT(rs.csr1,dfc) sqrt(rs.csv1./f)],.1)));
        axis([0 maxf -inf inf]);
        legend('Signal','Noise');
        xlabel('Frequency, pixel^{-1}','fontsize',12);
        ylabel('Amplitude','fontsize',12);
        set(gca,'fontsize',12);
        
        subplot(325);
        set(gca,'fontsize',12);
        plot(f0,sqrt(rs.csnr1/dsw)/np);
        axis([0 maxf -inf inf]);
        xlabel('Frequency, pixel^{-1}','fontsize',12);
        ylabel('Localization L, pixel^{-1}','fontsize',12);
        
        subplot(324);
        w0=rs.w0;
        p2=find(rs.csnr2>snr2Thresh^2,1,'last');
        w0(p2:np)=0;
        w0=w0/max(w0);
        w0=max(w0,eps);
        plot(f0,GaussFiltDCT(w0,dfc));
        axis([0 maxf -inf inf]);
         xlabel('Frequency, pixel^{-1}','fontsize',12);
                 ylabel('Filter weight W','fontsize',12);

        set(gca,'fontsize',12);
        subplot(326);
        plot(f0,sqrt(rs.csnr2)); hold on;
        plot(f0,rs.csnr2*0+snr2Thresh,'k--'); hold off;
        axis([0 maxf -inf inf]);
        xlabel('Frequency, pixel^{-1}','fontsize',12);
        ylabel('SNR \sigma_2','fontsize',12);
         set(gca,'fontsize',12);
