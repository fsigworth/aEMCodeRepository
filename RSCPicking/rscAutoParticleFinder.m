...
if max(rscc.mxVesInds(:))>numel(mi.vesicle.x)  % out of bounds vesicle numbers
    error('Autopicking aborted: mismatch between vesicle picker and preprocessing');
end;
...
%                     the pick.
                    sval=rspResidualSpectrum(mi,rscc,dis,partCtr,0);
                    if sval<sThresh
                        nFound=nFound+1;
                        coords(nFound,:)=[ partCtr 32 single(iv) amp templ rso sval ampU];
                    end;
                end;
            elseif dis.spMode
                templ=single(rscc.mxTemplInds(ix,iy));
                ...
                if sval<sThresh
                    nFound=nFound+1;
                    coords(nFound,:)=[ partCtr 32 single(iv) amp templ alpha sval ampU];
                end;
