...
for i=vesInds
    ...
    mxVals mxInds var cImage iptrs mxRimg ccs
    [mxVals mxInds var cImageX iptrs mxRimg ccs]=rsVesicleCorrelation(-m2,mic,i,...
        membraneOffset,partRadius,angleInds,eigenSet,1);
        r=mi.vesicle.r(i)/ds-membraneOffset;
        ctr=[mic.vesicle.x(i) mic.vesicle.y(i)]/ds+1;  % shift to 1-based coords
    vesMsk=fuzzymask(n,2,r,1,ctr);
    mxVals2=mxVals.*vesMsk;
    [amp ix iy]=max2d(mxVals2);
return    
    while amp>minAmp
        itempl=mxInds(ix,iy);
                %     Create a full fake image and correlate it.
        resRef=xTemplates(:,:,itempl)*mic.vesicle.s(i);
        mx=ExtractImage(resRef,[ix iy],n,1);
        [txVals txInds]=rsVesicleCorrelation(mx,mic,i,membraneOffset,...
            partRadius,angleInds,eigenSet,1);
        localCC=ExtractImage(mxVals,[ix iy],nl).*mskl;
...
%         imacs(localModel);
        ...
        npts=sum(sum(abs(localCC)<.05))/100;
%          Blank the spot in the CC
        mxVals2=mxVals2.*(1-fuzzymask(n,2,blankRadius,2,[ix iy]));
        subplot(224);
        imacs(mxVals2);
        drawnow;
        disp([i amp ccVar residVar ix iy])
        if (amp > minAmp && amp < maxAmp && residVar < maxVar)
            nFound=nFound+1;
            ctrs(nFound,:)=[ix iy];
        end;
%         pause
