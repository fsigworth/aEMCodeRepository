...
%     ves=meMakeModelVesicles(mi,n,1:1000,0);  % no ctf
    m1=m0-ves;
end;
...
nl=48;
msklRadius=17;
    mskl=fuzzymask(nl,2,msklRadius,2);
            npts=sum(mskl(:));
...
nves=numel(vesInds);
if useParallel
    xValsPar=single(zeros(n,n,nves));
    xIndsPar=single(zeros(n,n,nves));
    xVarsPar=single(zeros(n,n,nves));
end;
parfor j=1:nves
    i=vesInds(j);
    maskRadii=mic.vesicle.r(i)/ds+membraneOffset*[-1 1 0];
    maskRadii(3)=maskRadii(3)+abs(membraneOffset)+msklRadius;
    ...
    % Do a local averaging of the variance in the whole vesicle.
    var=mxValsV.^2;
    h=ifftshift(Crop(mskl,nv));
    filtVar=real(ifftn(fftn(var).*fftn(h))).*fuzzymask(nv,2,max(maskRadii(1:2)),.5);
%         figure(2);
%         subplot(2,2,1);
%         imacs(mxVals);
     mxVals2=mxVals;
    [amp ix iy]=max2d(mxVals);
%     while amp>minAmp
%         localCC=ExtractImage(mxValsV,[ix iy],nl).*mskl;
%         ccVar=localCC(:)'*localCC(:);
%         ccVar2=filtVar(ix,iy);
%         localImg=ExtractImage(filtImg,[ix iy],nl).*mskl;
%         localVar=localImg(:)'*localImg(:);
%         result=[i amp round(ccVar) round(ccVar2*.93) round(localVar/20) [ix iy]+round(ctr)];
%         disp(result)
%         k=k+1;
%         results(k,:)=result;
%         if (amp > minAmp && amp < maxAmp && ccVar < maxVar)
%             nFound=nFound+1;
%             ctrs(nFound,:)=round(ctr)+[ix iy]-ceil((nv+1)/2);
%         end;
%         mxVals2=mxVals2.*(1-fuzzymask(nv,2,partRadius,1,[ix iy]));
% %         subplot(2,2,2);
% %         imacs(mxVals2);
% %         title(k);
% %         drawnow;
%         [amp ix iy]=max2d(mxVals2);
%     end;
    
    %     Merge it into the mxCC and mxGamma images
    nv=size(filtImg,1);
    vesMsk=fuzzymask(nv,2,max(maskRadii(1:2)),1,ctr);
    vesVar=filtImg.^2.*vesMsk;
    var=ExtractImage(vesVar,ctr,n,1);
    xVals=ExtractImage(mxVals,ctr,n,1);
    xInds=ExtractImage(mxInds,ctr,n,1);
...
if useParallel
    [mxCC q]=max(xValsPar,[],3);
    mxVesInds=xIndsPar(:,:,q);
    [mxVars q2]=max(xVarsPar,[],3);
