...
% pixNorm=sqrt(localVarPix);  % removed
% ccPixNorm=ccPix./repmat(pixNorm,1,nterms);  % removed: like ccPix, but divided by sigma_img
...
nccPixMasked(2:2:nRefs,:)=repmat(-inf*(mski(pix')<=.5),nHemi*nGamma,1);

% msksV=mskv(pix)';
% cimgV=cimg.*repmat(msksV,nRefs,1)./paArray;  % whole-vesicle cross-correlation
% % not needed: nccVPixMasked=nccPix.*repmat(msksV,nRefs,1);  % whole-vesicle cross-correlation
...
% denormNCC=mxNCC'.*pixNorm;  % removed
denormNCC=mxNCC';
ampPix=zeros(npix,1);
for i=1:npix
