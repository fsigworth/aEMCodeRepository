% DoMergeImagesHS
% merge Hideki's double exposures
...
pa='/Volumes/TetraData/EMWork/Hideki/110628/HS12slot4_GluR2-GFP';
% pick up the path
...
while (name(1)==('.' || ~strcmpi(ex,'.dm3'))&& di<nd
    di=di+1;
    name1=d(di).name;
    [pa nm ex]=fileparts(name);
end;
...
a1=sscanf(name1,formatString);
a2=sscanf(name2,formatString);
