...
% v3.0  11 Apr 2014
% Definitions for the first data structures only are updated
...
stackInfo
    si.pixA         % Copied from runInfo, should match the mrc stack
    si.mbnOffset    % Negative for particle with mass outside.
    ...
    si.sVesicle(i)  % Copied from the mi struct
    si.mi{j}        % Copy of the whole mi structures
    si.ctfs(n,n,j)  % Zero-center ctf for each mi
%     Note that all dimensions are given in units of pixA.


%% REFINEMENT RUN
% Description of the run: iterations with the same set of basic parameters
runInfo
    ri.n         % current working image size
    ri.radius    % limiting mask radius
    ri.nTrans    % number of translation steps away from zero
    ri.softMask  % 2D mask for particles
...
%     The following are set up in reSetRefAngles
    ri.angleStep(3)
    ri.angleMin(3)
    ri.angleN(3)
