function logProbWC=reComputeLogProbWC3(nt,vR,yClick,mbnOffset,ri,moi,imgIndex)
% function logProbWC=reComputeLogProbWC3(nt,vR,yClick,mbnOffset,ri,moi,imgIndex);
% Derived from logProbWC2 but now computes only those ntc translations
...
if ~isfield(moi,'activeTrans')
    ...
    activeT=moi.activeTrans(:,:,imgIndex);
end;
tc=reshape(activeT,nt*nt,1);
