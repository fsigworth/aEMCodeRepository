function logProbWC=reComputeLogProbWC2(nt,vR,yClick,mbnOffset,ri,moi,imgIndex)
% function logProbWC=reComputeLogProbWC2(nt,vR,yClick,mbnOffset,ri,moi);
...
% Gaussian.  Returns an nt x nt x nAlphas x nIos x nGammas x nBetas set of
...
% ri.angleN=[5 18 9];
% ri.angleMin=[-40 0 0];
% ri.angleStep=[10 10 20];
% ri.alphas=ri.angleMin(1)+(0:ri.angleN(1)-1)*ri.angleStep(1);
% ri.alphas=[ri.alphas ri.alphas+180];
% ri.isos=[0 1];
% mbnOffset=10;
% tic
%
...
[x0,y0]=ndgrid(-nh:nh,-nh:nh);  % Make zero at ctr,ctr
X=zeros(nt,nt,nAlphas,nIsos,'single');
Y=zeros(nt,nt,nAlphas,nIsos,'single');
if nt>1
    logProb=zeros(nt,nt,nAlphas,nIsos,1,nBetas);
...
if nt>1
    logProbWC=repmat(logProb,1,1,1,1,nGammas,1);
end;
