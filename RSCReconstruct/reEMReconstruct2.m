...
disp('PBS environment:');
s=getenv('PBS_ENVIRONMENT')
isCluster=numel(s)>0;
isInteractive=~strcmp(s,'PBS_BATCH') || ~isCluster;
nodeNum=str2double(getenv('PBS_ARRAYID'))

% f=fopen(['txt' num2str(nodeNum)],'w');
% fprintf(f,'output %02d',nodeNum);
% fclose(f);


% cd /fastscratch/fjs2/140625
...
ri.nIters=20;
ri.nIters=2  %%%%%%%%%%%
ri.names={'sq10_350p128tsi.mat'};
%ri.names=[];

ri.stackPath='Stack/';

ri.outPath='Reconstructions/Recon40c1/'; % relative to the root path
ri.prefix='150105';
ri.kFactor=1;
...
angleSteps=[5 5 10]
% angleSteps=[2 2 4]    % step sizes in degrees
angleLimits=[10 0 360/symmetry];  % starting alpha and beta; range of gamma.
% isos=[0 1];
isos=1;
doNormalize=1;
...
if ri.fscFlag<0  % pick up odd or even images
    ...
    nac0=min(activeFlagIndex,nac0);
    si0.activeFlags(:,nac0+1)=si0.activeFlags(:,nac0);
    startInd=max(1,-ri.fscFlag);  %fscFlag=-1 means odd, -2 means even are kept
inds=find(si0.activeFlags(:,end));  % pick out the alternate active images.
inds=inds(startInd:2:numel(inds));
si0.activeFlags(:,end)=false;
si0.activeFlags(inds,end)=true;
...
%%    % Display volumes and fscs
    fscMask=fuzzymask(n0,3,fscMaskRadius,fscMaskWidth);
...
%     fracActiveRefs=sum(roi.pRefs(:)>transThreshold)/numel(roi.pRefs)
    ...
    if flagWriteVolumes
        ...
        save(outNameAll,'outputVols','mois','rois','si','ri');
    end;
