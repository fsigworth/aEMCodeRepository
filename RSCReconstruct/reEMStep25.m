function roi=reEMStep25(imgs,refs,si,ri,moi,altImgs)
% function roi=reEMStep25(imgs,refs,si,ri,moi,altImgs)
...
badAI=[];
for i=1:nIsos
    badAI=[badAI [1 nAlphas]+nAlphas*(i-1)];
end;
badTy=[1 nt];
...
for iMi=1:nMi
    ...
    for iImg=miPartIndices{iMi} % all the particles from this micrograph
        ...
        if imgAmp>0 && any(tc(:))  % This image is active
            ...
            if ri.usePrior
                ...
                % size.
                logProbWC=reComputeLogProbWC4(nt,vesR,y0,si.mbnOffset,ri,moi,iImg);
                ...
                for iVol=1:nVols     % expand width by nVols
                    if numel(moi.pRefs)>1
                        wcPriors(:,:,iVol)=reshape(logProbWC,ntAI,nRefs)...
                            +log(moi.pVols(iVol)+repmat(moi.pRefs(:,iVol,iImg)',ntAI,1));
                    else  % is this right??
                        wcPriors(:,:,iVol)=reshape(logProbWC,ntAI,nRefs)...
                ...
                if makeRawProbs
                    ...
                    rawLogProbs.pXs(tc,:,:,iImg)=-reshape(diffIR,ntc,nAI,nRV)/(2*moi.sigmaN^2);
                end;
            ...
            if ~(any(tx==badTx) || any(ty==badTy) || any(indAI==badAI))
                % Accumulate the class means
                pixMeans(:,:)=pixMeans(:,:)+eImg_tai_rv*imgAmp;
                rawNorms=rawNorms+imgAmp^2*prv_X(:);  % nRV x 1
            else
    ...
    classNorms=classNorms+kron(ctf(:).^2,rawNorms'); % nw^2 x nRV
end; % for iMi
