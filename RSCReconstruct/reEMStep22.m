function [classMeans,classNorms,roi,imgBestMatch,rawMeans,rawNorms,rawLogProbs]...
    =reEMStep22(imgs,refs,si,ri,moi,altImgs)
...
logPX=zeros(1,nImgs);  % note that unassigned values are zero!
imgAmps=zeros(1,nImgs,'single');
eVar=zeros(1,nImgs,'single');
pVols=zeros(nVols,nImgs);
pt_Xs=zeros(nt2,nImgs,'single');
pt_X0s=zeros(nt2,nImgs,'single');
bmClass=zeros(1,nImgs,'single');  % best-matching ref/volume
bmTAI=zeros(1,nImgs,'single'); % best matching trans/alpha/iso
...
%********
if makeRawProbs
    rawLogProbs.priors=zeros(nTAI,nRV,nImgs,'single');
    rawLogProbs.pXs=zeros(nTAI,nRV,nImgs,'single');
end;
%*********
...
for iImg=1:nImgs
    ...
    if imgAmp>0 % This image is active
...
%           Get the power from an image with angle near 0
        ...
        transImgs=zeros(nPix,nt2,nAI);  % 1D images nTrans*nAlpha
        %         Make the translated images, multipled by soft mask
        for i=1:nAI  % loop over alphas
            uImg=unrotImgs(:,:,i);
            uImg=uImg(:);
            for j=1:nt2  % loop over translations
                transImgs(:,j,i)=uImg(transPix(:,j)).*softMaskPix;
            end;
        end;
...
%         ------set up for altImgs------
        if useAltImgs  % in case we're using alternative images for recon
            ...
            transAltImgs=zeros(nPix,nt2,nAI);
            for i=1:nAI  % loop over alphas
                uAltImg=unrotAltImgs(:,:,i);
                uAltImg=uAltImg(:);
                for j=1:nt2  % loop over translations
                    transAltImgs(:,j,i)=uAltImg(transPix(:,j)).*softMaskPix;
                end;
            end;
...
%                               img and ref, nt2*nAlphas*nIsos, nRefs*nVols
        
        % Create the WC log priors, which will be nTAI x nRefs in
        % size.
        logProbWC=reComputeLogProbWC2(nt,vesR,y0,si.mbnOffset,ri,moi);
        ...
        if makeRawProbs
            rawLogProbs.priors(:,:,iImg)=wcPriors;
            rawLogProbs.pXs(:,:,iImg)=-diffIR/(2*moi.sigmaN^2);
        end;
...
%         Get the full latent probabilities, p(t,a,i,r,v | X)
        ...
        pt_X0=sum(reshape(ptai_X,nt2,nAI),2);  % no rotations
        pAlphasI(:,iImg)=sum(reshape(ptai_X,nt2,nAI),1)'; % p(alphaI): sum over trans
...
%         Get the best matches
        [~, bmTAI(iImg), bmClass(iImg)]=max2d(ptairv_X);  % best matches
        pixBestMatch(:,iImg)=transImgs(:,bmTAI(iImg));
...
%         Accumulate other model parameters
        ...
        pt_Xs(:,iImg)=pt_X;   % prob(translations)
        pt_X0s(:,iImg)=pt_X0; % prob(translations) without compensating rotation
...
roi.imgAmps=imgAmps;
roi.pTransUnrot=reshape(pt_Xs,nt,nt,nImgs);
roi.pTrans=reshape(pt_X0s,nt,nt,nImgs);
roi.varNs=single(eVar);
roi.pVols=pVols;
roi.logPX=logPX;
roi.pRefs=reshape(pRefs,nRefs,nVols,nImgs);
roi.pAlphas=pAlphasI;

% Best-match parameters
roi.imgClass=bmClass;  % best match ref for each image
[tx, ty, ia]=ind2sub([nt nt nAI],bmTAI); % best match trans and alpha
roi.imgTA=([tx; ty; ia].*repmat(bmTAI~=0,3,1));  % ...zeros when not assigned
