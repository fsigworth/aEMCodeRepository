function [logP, accumImg, accumPars, pAlphas, pTrans]=reOneImgOneRef(fUnrotImgs,logProbWC,sigmaN, ref)
% function [logP, accumImg, accumPars, pAlphas, pTrans]=rsOneImgOneRef6(fUnrotImgs,logProbWC,sigmaN, ref)
% Inputs:
...
%   ref:        n x n image, already scaled by amplitude a * ctf
% Outputs:
...
for i=1:nAlphas
    ...
    diffIR=(I2+R2-2*ccIR);%-ns*ns*log(sqrt(2*pi)*SigmaN)
    %         Get log(P(I|w,alpha,...))
    ...
    % amp estimate = img * (a_old*ref)/(a_old^2 * ref^2), needs to be
    % multiplied by a_old externally.
