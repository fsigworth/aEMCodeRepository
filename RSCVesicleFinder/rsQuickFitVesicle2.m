function [miNew, imgc, vesFit]=rsQuickFitVesicle2(img,oldVes,mask,mi,vIndex,effCTF,hfVar,nRTerms,rConstraints,displayOn)
% function [miNew,imgc,vesFit]=rsQuickFitVesicleN(img,oldVes,mask,mi,vIndex,effCTF,hfVar,nRTerms,displayOn)
% Given a subtracted image, restore the original vesicle oldVes and fit by
...
% The nRTerms is an array of nRounds x 2 elements.
% if nRTerms(i,1)=0 fit amplitudes only
% nRTerms(i,1)=1 fit radius too.
% nRTerms(:,1)>1 include higher-order radius terms
% nRTerms(:,2) elements specify the number of terms to use in
% the expansion of the amplitude vesicle.s(vIndex).  If missing, these values are
% determined from nRTerms(:,1).
...
% initStepA=150;  % initial step is ~ membrane width %%%%%%
...
if nargin<8
    nRTerms=1;  % don't fit radius
end;

mask=single(mask);

if size(nRTerms,2)<2 % only gave the first column which are radius
    nRTerms(:,2)=1+floor(nTerms(:,1)/2); % number of amplitude terms
end;
...
% nt1=3,5,7...nTerms
stepFraction=0.25; % decrement of magnitudes of higher terms

nrt1=nRTerms(end,1); % Final number of radius terms
nPars=max(0,1+(nrt1-2)*2);  % overall number of simplex parameters
nRounds=size(nRTerms,1);  % number of simplex rounds
...
%     nrt1=1;
% end;
vr=vr0(1:nrt1); % copy at most nt1 values into vr, the working radius vector.

% frac=.5; % decrement of starting steps for radial terms
frac=1; % decrement of starting steps for radial terms
startingSteps(1)=initStepA/pixA;
for i=2:2:nPars
    startingSteps(i:i+1)=startingSteps(1)*frac/i;
%     startingSteps(i:i+1)=startingSteps(1)*frac;
end;
% startingSteps(2:7)=startingSteps(1); %%%%
...
p.vr(2:end)=0;  % initialize to round.  %%%%%%
p.vr0=vr0;  % full radius vector as read from mi file, to fill in missing vr elements.
p.effCTF=effCTF;
p.mask0=mask1;
p.ccMask=ccMask;
p.logPScaled=logP*hfVar/vs;  % scale up to match ccf
...
for iRound=1:nRounds % we'll do one round if nTerms(1)=0
    nTermsRound=nRTerms(iRound,1);
    p.nTerms=nRTerms(iRound,:);
    p.rConstraints=rConstraints(1:p.nTerms(1));
    ...
    if nTermsRound>0 % we are doing nonlinear fitting
        ...
        [p.vr,s,t,err,vesFit]=SimplexFit(p,nIters(iRound),useGPU);
        %         p.vr=vr1;  % update the starting radius
    else % Just doing a linear fit of amplitudes
        p.vr=p.vr0;
        [vesFit,s,err]=AmpFitNX(p,useGPU);
        p.simpMask=[];
    end;
...
miNew=mi;
sh=approxLoc-ctr;
if nRTerms(1,1)>0  % we fitted position and radius
    t=gather(t);
    miNew.vesicle.r(vIndex,:)=0;
    miNew.vesicle.r(vIndex,1:nrt1)=p.vr*ds;   % complex radius
...
miNew.vesicle.s(vIndex,1:nRTerms(end,2),1:nxc+1)=gather(s);
...
function [vR,s,t,err,vFit]=SimplexFit(p,nIters,useGPU)
...
%     end;
    ...
    [vfit,a,t]=LLSFit(p,useGPU); % Get the amplitude and translation fit
    p.a=a;
    p.t=t;
    res=p.imgc(:)-vfit(:);
    err=res'*res;
    %               Simplex update
    P1=Simplex(boundsErr+gather(err));  % update the vesicle radius
...
P1=Simplex('centroid');
p.vr=Pack(P1,p.nTerms);
[vFit,a,p.t]=LLSFit(p,useGPU); % Get the translation one last time
vR=p.vr;
[vFit,s]=AmpFitNX(p,useGPU);
res=p.imgc(:)-vFit(:);
err=res'*res;
t=p.t;
end
...
function [vfit,a,t]=LLSFit(p,useGPU)
...
if useGPU
    v=-gpuVesicleFromModelGeneral(ndis,p.vr,p.vd,p.t);
    F=ones(ndis^2,2,'single','gpuArray');
else
...
function [vfit,s,err]=AmpFitNX(p,useGPU)
...
ndis=size(p.imgc,1);
if useGPU
    vArray=zeros([ndis ndis nx+1],'single','gpuArray');
    vArray(:,:,1)=-gpuVesicleFromModelGeneral(ndis,p.vr,p.vd,p.t);
else
...
if useGPU
    if nt>1
        vArray(:,:,2:nt)=-p.pixA*gpuVesicleFromRings(ndis,exPos,exSigma,p.vr,p.t);
    end;
    Ndis=gpuArray(ndis);
    F=zeros(Ndis^2,(2*nsTerms-1)*nt,'single','gpuArray');
else
    if nt>1
...
if nsTerms==1
    ...
    if useGPU
        s=zeros(nsTerms*nt,1,'single','gpuArray');
    else
