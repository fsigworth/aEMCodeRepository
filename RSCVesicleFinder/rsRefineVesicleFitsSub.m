...
%
% Mark bad amplitudes as null vesicles.
% % miNew.vesicle.s=zeros(nVes,nSTerms,nPeaks+1,'single');
nPeaks=numel(miNew.vesicle.extraPeaks);

sVals=miOld.vesicle.s(:,1,1);  % Copy the basic amplitude
sVals(isnan(sVals)|sVals<tinyValue)=0;
miNew.ok(sVals==0,:)=0;  % zero or negative assigned to be null vesicles.

maxNTerms=numel(pars.nTerms);
nSTerms=maxNTerms;
...
nvToFit=min(nvToFit,maxVesiclesToFit);
...
% for j=84
    ...
    if ~ok && useOkField
        ...
        for i=1:nRounds
            nTR(i,1)=max(origNTerms,ceil(i*nTerms(1)/3));
            nTR(i,2)=min(nTerms(2),ceil(i*nTerms(2)/3));
        end;
